...
if imagen1.size != imagen2.size:
    ...
    imagen2_not = Image.new('L', imagen2.size)
    ...
    for x in range(ancho):
        for y in range(alto):
            pixel1 = imagen2_neg.getpixel((x, y))
            color = not pixel1
            if color == 0:
                imagen2_not.putpixel((x, y), 0)
            else:
                imagen2_not.putpixel((x, y), 255)

    for x in range(ancho):
        for y in range(alto):
            pixel1 = imagen1_neg.getpixel((x, y))
            pixel2 = imagen2_not.getpixel((x, y))
            color = pixel1 or pixel2
            if color == 0:
